...
namespace Cosmos
{
    ...
    void ResistorCalcWidget::Update()
    {
        ...
        ImGui::Begin("Resistor Calculator", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar);
        {
            ...
            ImGui::SetCursorPos(ImVec2(pos.x + 40.0f, pos.y + 40.0f));
            ImGui::NewLine();
    ...
    void ResistorCalcWidget::DrawInfo()
    {
        ...
        if (WidgetCentered(ImGui::Button(title, ImVec2(ImGui::CalcTextSize(title).x + 20.0f, 50.0f)))) { ImGui::OpenPopup("InfoPopup"); }

        ImGui::SetCursorPosY(ImGui::GetContentRegionAvail().y);

        if(WidgetCentered(ImGui::Button("Quit", ImVec2(150.0f, 50.0f)))) {
            mApp->Quit();
        }
