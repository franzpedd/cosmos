...
namespace Cosmos
{
    ...
    void ResistorCalcWidget::Update()
    {
        ...
        ImGui::Begin("##CoreWidgetWindow", nullptr, ImGuiWindowFlags_NoDecoration);
        {
            ...
            constexpr float yValue = 100.0f;
            #else
            constexpr float yValue = 50.0f;
            #endif

            // title and it's popup
            const char* title = "ResCalc (Resistor Calculator)";

            if (WidgetCentered(ImGui::Button(title, ImVec2((ImGui::CalcTextSize(title).x + 20.0f), yValue)))) { ImGui::OpenPopup("InfoPopup"); }
            ...
            // main content
            ImGui::BeginChild("##ContentChild", ImVec2(size.xy.x, size.xy.y - 200.0f));
            {
                ...
                ImGui::SeparatorText("Resistor stripes " ICON_LC_MESSAGE_CIRCLE_QUESTION);
            ...
            // quit button
            if (WidgetCentered(ImGui::Button("Exit", ImVec2(150.0f, yValue))))
            {
                mApp->Quit();
            }
        }
    ...
    void ResistorCalcWidget::CalculateResistance(int bandMode)
    {
        ...
        switch (bandMode)
        {
            case 0: // 3 bands
            {
                ...
                mResistorInfo.msg << formatted << " ±20%";
                break;
            }
            ...
            case 3: // 6 bands
            {
                ...
                char formatted[32];
                Resistance_Format(res, formatted);
                mResistorInfo.msg << formatted << " " << ICON_LC_DIFF << GetToleranceValue(mTolerance) << "%  " << GetTemperatureCoefficient(mTemperature);
                break;
            }
    ...
    void ResistorCalcWidget::Resistance_Format(double ohms, char* output)
    {
        ...
        else if (ohms >= 1.0) sprintf(output, "%.2f " ICON_LC_OMEGA, ohms);
        else if (ohms >= 0.001) sprintf(output, "%.2f m" ICON_LC_OMEGA, ohms * 1000);  // milliohms
        else sprintf(output, "%.2f " ICON_LC_OMEGA, ohms);  // fallback
    }
